...
namespace rind {
    ...
    public:
        Missile(
            ...
            float lifetime = 5.0f
        ) : engine::Entity(entityManager, "missile" + hashName(transform), "gbuffer", transform, {"materials_slowbullet_albedo", "materials_slowbullet_metallic", "materials_slowbullet_roughness", "materials_slowbullet_normal"}, true, engine::Entity::EntityType::Generic), velocity(velocity), timeRemaining(lifetime), color(color), player(player) {
            setModel(entityManager->getRenderer()->getModelManager()->getModel("slowbullet"));
            setTransform(glm::scale(transform, glm::vec3(0.5f)));
        ...
        void update(float deltaTime) override {
            ...
            if (player) {
                glm::vec3 toPlayer = player->getWorldPosition() - getWorldPosition();
                float distanceToPlayer = glm::length(toPlayer);
                if (distanceToPlayer > 0.1f) {
                    toPlayer /= distanceToPlayer;
                    float turnSpeed = 5.0f; // radians per second
                    glm::vec3 currentDir = glm::normalize(velocity);
                    float angleDiff = std::acos(glm::clamp(glm::dot(currentDir, toPlayer), -1.0f, 1.0f));
                    if (angleDiff > 0.01f) {
                        float maxTurn = turnSpeed * deltaTime;
                        if (angleDiff < maxTurn) {
                            currentDir = toPlayer;
                        } else {
                            currentDir = glm::normalize(currentDir + toPlayer * (maxTurn / angleDiff));
                        }
                        velocity = currentDir * glm::length(velocity);
                    }
                }
            }
