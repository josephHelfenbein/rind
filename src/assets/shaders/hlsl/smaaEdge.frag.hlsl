...
float4 main(VSOutput input) : SV_Target {
    ...
    float Lleft = dot(colorTexture.Sample(sampleSampler, input.fragTexCoord + float2(-pc.invScreenSize.x, 0)).rgb, weights);
    float Ltop  = dot(colorTexture.Sample(sampleSampler, input.fragTexCoord + float2(0, -pc.invScreenSize.y)).rgb, weights);
